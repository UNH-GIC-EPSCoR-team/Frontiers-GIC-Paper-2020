...
magData_train.drop(labels=['MLT','IGRF_DECL', 'Date_UTC.1'], axis=1)

print(magData_train.columns)

omni_drop = ['PC_N_INDEX', 'BY_GSE', 'BZ_GSE',
...
create_average_delays(omniData_train, omniData_train.B_Total.name, time=time_steps)
create_average_delays(omniData_train, 'T', time=time_steps)
create_average_delays(omniData_train, omniData_train.flow_speed.name, time=time_steps)
create_average_delays(omniData_train, omniData_train.Pressure.name, time=time_steps)
...
X = trainData.drop(labels=['N', 'E', 'Z', 'MAGNITUDE', 'dBT','Date_UTC.1', 'MLT','IGRF_DECL', 'MLAT'], axis=1)
del trainData
...
train_gen = Generator(X_train, y_train, batch_size=512)
val_gen = Generator(X_val, y_val, batch_size=512)
...
srunTime = time.time()
model = Sequential()
model.add(LSTM(147, activation='relu', input_shape=(n_steps, n_features)))
model.add(Dense(1))
...
# model = load_model('../models/multi_LSTM_w_avgs.h5')
...
magData_2015.drop(labels=['MLT','IGRF_DECL', 'Date_UTC.1'], axis=1)
magData_2011.drop(labels=['MLT','IGRF_DECL', 'Date_UTC.1'], axis=1)
...
create_average_delays(omniData_2015, omniData_2015.B_Total.name, time=time_steps)
create_average_delays(omniData_2015, 'T', time=time_steps)
create_average_delays(omniData_2015, omniData_2015.flow_speed.name, time=time_steps)
create_average_delays(omniData_2015, omniData_2015.Pressure.name, time=time_steps)
...
create_average_delays(omniData_2011, omniData_2011.B_Total.name, time=time_steps)
create_average_delays(omniData_2011, 'T', time=time_steps)
create_average_delays(omniData_2011, omniData_2011.flow_speed.name, time=time_steps)
create_average_delays(omniData_2011, omniData_2011.Pressure.name, time=time_steps)
...
train2015 = train2015.drop(labels=['N', 'E', 'Z', 'MAGNITUDE', 'dBT','Date_UTC.1', 'MLT','IGRF_DECL', 'MLAT'], axis=1)
train2011 = train2011.drop(labels=['N', 'E', 'Z', 'MAGNITUDE', 'dBT','Date_UTC.1', 'MLT','IGRF_DECL', 'MLAT'], axis=1)
date_2015 = train2015['Date_UTC']
date_2011 = train2011['Date_UTC']

print(train2015.columns.tolist())
print(train2011.columns.tolist())
...
predict2015, pod2015, pofd2015, pc2015, hss2015 = get_results(test2015, yhat_2015, date_2015, thresholds, window=20)
predict2011, pod2011, pofd2011, pc2011, hss2011 = get_results(test2011, yhat_2011, date_2011, thresholds, window=20)
