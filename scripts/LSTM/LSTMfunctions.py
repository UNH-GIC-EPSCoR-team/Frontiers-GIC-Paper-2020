...
def create_average_delays(df, name, time=12):
    for delay in np.arange(20,int(time)*10+1,10):
...
def get_results(real, predicted, date, thresholds, window):
    ...
    d['date'] = date
    pod, pofd, pc, hss = classification_scores(d, thresholds, window)
    d.set_index('date', inplace=True, drop=True)
    d.index = pd.to_datetime(d.index)
    # predictions = pd.DataFrame({'POD': pod,
    #                             'POFD': pofd,
    #                             'PC':pc,
    #                             'HSS': hss,
    #                             index=thresholds})
...
def classification_scores(df, thresholds, window):
